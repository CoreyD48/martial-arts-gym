...
### Changed
- **`src/app/(auth)/layout.tsx`** — Dark gradient background with indigo radial glow and subtle grid pattern replaces plain gray.
- **`src/app/(auth)/login/page.tsx`** — Login card restyled: dark glassmorphism card, white text on dark inputs, indigo logo mark above the form, polished button shadow.
