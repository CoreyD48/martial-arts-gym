...
export async function middleware(request: NextRequest) {
  ...
  // This is the endpoint NextAuth hits when a user submits the login form.
  if (
    request.method === "POST" &&
  ...
  // Delegate route-protection (auth + role checks) to proxy.
...
export const config = {
  matcher: [
    "/dashboard/:path*",
    "/portal/:path*",
    // Must include the credentials callback to intercept login POSTs.
